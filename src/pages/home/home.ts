...
export class HomePage {
  ...
  masterListAddSelectClass() {
    ...
    setTimeout(() => {
      ...
      for (var i = 0; i < this.checkedTraits.length; i++) {
        jQuery(".singleTraits." + this.checkedTraits[i].traituniqueid).addClass(
          "checkedStyle"
        );
        jQuery(
          ".masterListStyle." + this.checkedTraits[i].traituniqueid
        ).addClass("checkedStyle");
      }
    }, TIME_IN_MS);
  }
  ...
  openTraitsList() {
    this.navCtrl.push("TraitsListPage", {
      dataFor:
        this.frdInfo && this.frdInfo.id != 0
          ? "addTraitForFrd"
          : "addTraitForOwn",
      ...
      if (this.dataFor == "addTraitForFrd") {
        if (this.checkedTraits.length < this.numTraitToFrds) {
          ...
          this.traitService.presentSuccessToast(
            `you can give only ${this.numTraitToFrds} traits`
          );
        }
      } else {
        if (this.checkedTraits.length < this.numTraitToFrds) {
          ...
          this.traitService.presentSuccessToast(
            `you can select only ${this.numTraitToFrds} traits`
          );
        }
      }
    } else {
  ...
  addToPage() {
    if (this.dataFor == "addTraitForFrd") {
      ...
      this.traitService
        .addTraitToPage(this.checkedTraits, this.authToken)
        .subscribe(data => {
